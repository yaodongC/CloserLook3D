...
from utils.config import config, update_config, edict2dict
...
def parse_option():
    parser = argparse.ArgumentParser("Training and evaluating S3DIS")
...
def training(config):
    with tf.Graph().as_default():
        ...
        # saver = tf.compat.v1.train.import_meta_graph(config.load_path)
        ...
        if config.load_path is not None:
            ...
            saver.restore(sess,tf.train.latest_checkpoint(config.load_path))
        ...
        for epoch in range(1, config.max_epoch + 1):
            ...
            wandb.log({"Epoch Time": tic2 - tic1, "Learning Rate": lr})
    ...
    while True:
        try:
            ...
            if (batch_idx + 1) % config.print_freq == 0:
                ...
                wandb.log({"Train Loss": loss_meter.val, "Train Avg Loss": loss_meter.avg,
                           "Train Seg Loss": seg_loss_meter.val,"Train Avg Seg Loss": seg_loss_meter.avg,
                           "Train Weight Loss": weight_loss_meter.val,"Train Avg Weight Loss": weight_loss_meter.avg})
            batch_idx += 1
...
def val_one_epoch(sess, ops, dataset, validation_probs, val_proportions, epoch):
        ...
        try:
            ...
            if (idx + 1) % config.print_freq == 0:
                logger.info(f'Val: [{epoch}][{idx}] '
                            ...
                            f'seg loss {seg_loss_meter.val:.3f} ({seg_loss_meter.avg:.3f}) '
                            f'weight loss {weight_loss_meter.val:.3f} ({weight_loss_meter.avg:.3f})')
                wandb.log({"Val Loss": loss_meter.val, "Val Avg Loss": loss_meter.avg,
                           "Val Seg Loss": seg_loss_meter.val,"Val Avg Seg Loss": seg_loss_meter.avg,
                           "Val Weight Loss": weight_loss_meter.val,"Val Avg Weight Loss": weight_loss_meter.avg})                
            idx += 1
        except tf.errors.OutOfRangeError:
            break
    ...
    wandb.log({"mIoU": mIoU, "IoUs": IoUs,"Vote mIoU": vote_mIoU, "Vote IoUs": vote_IoUs}) 
    return
...
def val_vote_one_epoch(sess, ops, dataset, epoch, num_votes=20):
    ...
    wandb.log({"Vote mIoU": mIoU, "Vote IoUs": IoUs})  
...
if __name__ == "__main__":
    ...
    wandb.init(project="point-cloud-operator", tags="S3DIS",config=edict2dict(config),resume=True)
