...
def training(config):
    with tf.Graph().as_default():
        ...
        is_training_pl = tf.placeholder(tf.bool, shape=()) # place holder
        ...
        for i, igpu in enumerate(config.gpus):
            with tf.variable_scope(tf.get_variable_scope(), reuse=True): # set reuse to true to share same model weights among GPUs
                with tf.device('/gpu:%d' % (igpu)), tf.name_scope('gpu_%d' % (igpu)) as scope: # use num_gpus as variable name_scope
                    flat_inputs_i = flat_inputs[i] # distribute data among gpus
        ...
        with tf.control_dependencies(update_ops):
            train_op = optimizer.apply_gradients(grads) # minimize error
    ...
    while True:
        try:
            ...
            if (batch_idx + 1) % config.print_freq == 0:
                ...
                wandb.log({"Train Acc": acc_meter.val, "Train Avg Acc": acc_meter.avg,
                           "Train Loss": loss_meter.val, "Train Avg Loss": loss_meter.avg,
                           "Train Class Loss": cls_loss_meter.val,"Train Avg Class Loss": cls_loss_meter.avg,
...
if __name__ == "__main__":
    ...
    wandb.init(project="point-cloud-operator", tags="ModelNet40",config=edict2dict(config),resume=False)
