...
def PosPool(config,
    ...
    Args:
        ...
        features: float32[n0_points, in_fdim] - input features of support points (3: coordinates)
    ...
    with tf.variable_scope(scope) as sc:
        ...
        # some shapes
        # input query points (center of neighborhoods)
        n_points = tf.shape(query_points)[0]
        # input support points (from which neighbors are taken)
        n0_points = tf.shape(support_points)[0]
        # indices of neighbors of each point
        n_neighbors = tf.shape(neighbors_indices)[1]
        # dimension of input features of support points
        ...
        if position_embedding == 'one':
            ...
            geo_prior = tf.maximum(1 - distances / tf.reduce_max(distances), 0.0)
            mid_fdim = 1
            shared_channels = fdim // 1
        elif position_embedding == 'linear2':
            geo_prior = tf.maximum(1 - distances / radius, 0.0)
            mid_fdim = 1
            shared_channels = fdim // 1
        ...
        # n_points: input query points (center of neighborhoods)
        # n0_points: input support points (from which neighbors are taken)
        # n_neighbors indices of neighbors of each point
        ...
        # print(aggregation_feature.shape)
        # # New part
        # append_relative_pos = True
        # if append_relative_pos==True:
        #     position_mat = relative_position  # [n_points, n_neighbors, 3]
        #     print(position_mat.shape)
        #     position_mat = tf.reshape(position_mat, [n_points, n_neighbors*3])  # [n_points, n_neighbors, 6*feat_dim]
        #     # print(position_mat.shape)
        #     ## position_mat = tf.expand_dims(alpha * position_mat, -1)  # [n_points, n_neighbors, 3, 1]
        #     out_pos_fdim = 3
        #     output_pos_features = conv1d_1x1(position_mat, out_pos_fdim, scope='relative_pos_conv',
        #                                  is_training=is_training,
        #                                  init=init, weight_decay=weight_decay,
        #                                  activation_fn=activation_fn, bn=bn, bn_momentum=bn_momentum,
        #                                  bn_eps=bn_eps)  # [n_points, out_pos_fdim]
        #     ## aggregation_feature ---- [n_points, fdim]
        #     ## output_pos_features ---- [n_points, out_pos_fdim]
        #     aggregation_feature = tf.concat( [output_pos_features, aggregation_feature],axis=-1)  # [n_points, fdim+out_pos_fdim]
        #     output_conv = True
